...
class send_messges:
    ...
    def send_data(self):
        ...
        data_bytes = struct.pack('fffifffi',
                                 ...
                                 self.angular_speed_z,
                                 self.model)
        rospy.loginfo("发送的字节：%s,%s,%s,%s,%s,%s,%s,%s",self.yaw,self.pitch, self.dis,self.linear_speed_x,self.linear_speed_y,self.angular_speed_z,self.shoot_model,self.model)
...
def decision(ser_msg_queue):
        ...
        while True:
            ...
            if begin!=1:
                ...
                """
                :param point: 要去的点
                :param time_need: 什么时间去
                :param time_begin: 比赛什么时候开始
                :param waiting_time:需要设置等待时间，等待时间应当小于等于两次命令间时常
                :return: 1 成功
                        0 时间未到
                        2 超时失败
...
if __name__ == '__main__':
    ...
    rospy.Subscriber('/rosdata', ROSdata, send.callback2,queue_size=10) #自瞄消息
    rate = rospy.Rate(20)  # max of frame 80-90
    
    nav = navigation()
    begin=0
    while not rospy.is_shutdown(): #决策
        send.model_decision()
        if begin!=1:
