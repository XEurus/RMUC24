...
def callback(data):
    ...
    #data_bytes = struct.pack('fff',angle_yaw,angel_pithc,distance,b'p',shoot_model,'ff',linear_speed,angular_speed,b'p',model )
    
    data_bytes =struct.pack('fffiffi',angle_yaw,angel_pithc,distance,shoot_model,speed,angle,model)
    rospy.loginfo("发送的字节：%s",data_bytes)
    #data_str = str(linear_speed) + ',' + str(angular_speed) + '\n'
